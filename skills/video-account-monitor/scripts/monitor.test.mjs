...
try {
  ...
  assert.ok(Array.isArray(xhsOptions.args));
  assert.ok(xhsOptions.args.includes("--disable-blink-features=AutomationControlled"));
  assert.ok(xhsOptions.args.includes("--no-sandbox"));
  assert.ok(xhsOptions.args.includes("--lang=zh-CN"));
  // Stealth args default to on
  assert.ok(xhsOptions.args.includes("--disable-features=IsolateOrigins,site-per-process,AutomationControlled"));
  assert.equal(xhsOptions.args.includes("--disable-web-security"), false);

  // Stealth disabled
  const noStealthOptions = buildBrowserLaunchOptions("kuaishou", { withStealthArgs: false });
  assert.equal(noStealthOptions.args.includes("--disable-features=IsolateOrigins,site-per-process,AutomationControlled"), false);

  // Web-security flag opt-in
  const webSecOptions = buildBrowserLaunchOptions("kuaishou", { withWebSecurity: true });
  assert.ok(webSecOptions.args.includes("--disable-web-security"));

  // resolveCdpEndpoint
  assert.equal(resolveCdpEndpoint(undefined), null);
  assert.equal(resolveCdpEndpoint(""), null);
  assert.equal(resolveCdpEndpoint(false), null);
  assert.equal(resolveCdpEndpoint("http://127.0.0.1:9222"), "http://127.0.0.1:9222");
  assert.equal(resolveCdpEndpoint("http://127.0.0.1:9222/"), "http://127.0.0.1:9222");
  assert.equal(resolveCdpEndpoint("  http://localhost:9333  "), "http://localhost:9333");
  assert.throws(() => resolveCdpEndpoint("127.0.0.1:9222"), /must be an http\(s\) URL/);
  assert.throws(() => resolveCdpEndpoint("ftp://127.0.0.1:9222"), /must be an http\(s\) URL/);
  assert.throws(() => resolveCdpEndpoint("http://127.0.0.1"), /explicit port/);
  assert.throws(() => resolveCdpEndpoint("http://[not-a-url]"), /not a valid URL/);
