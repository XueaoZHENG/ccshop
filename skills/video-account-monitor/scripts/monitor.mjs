...
function usage() {
  ...
  node scripts/monitor.mjs --platform kuaishou --account URL_OR_ID --cdp http://127.0.0.1:9222 --out ./outputs/name
...
Options:
  ...
  --cdp            Optional. Connect to an existing Chrome over CDP (e.g. http://127.0.0.1:9222). Reuses your logged-in session.
  --no-stealth     Optional. Disable the playwright-extra stealth plugin. Stealth is on by default.
...
export function buildBrowserLaunchOptions(platform, { withStealthArgs = true, withWebSecurity = false } = {}) {
  const args = ["--disable-blink-features=AutomationControlled"];
  if (withStealthArgs) {
    args.push("--disable-features=IsolateOrigins,site-per-process,AutomationControlled");
  }
  if (withWebSecurity) {
    args.push("--disable-web-security");
  }
  args.push("--no-sandbox", "--lang=zh-CN");
  return {
...
export function resolveCdpEndpoint(value) {
  ...
  if (!/^https?:\/\//i.test(text)) {
    throw new Error(`--cdp must be an http(s) URL (got "${value}"). Example: http://127.0.0.1:9222`);
  }
  let url;
  try {
    url = new URL(text);
  } catch {
    throw new Error(`--cdp is not a valid URL: "${value}".`);
  }
  if (!url.port) {
    throw new Error(`--cdp must include an explicit port. Example: http://127.0.0.1:9222`);
  }
...
async function loadPlaywright({ useStealth = true } = {}) {
  if (useStealth) {
    try {
      ...
      throw new Error(`Stealth mode requires playwright-extra and puppeteer-extra-plugin-stealth. Run \`npm install\` in the video-account-monitor skill directory. ${error.message || ""}`);
    }
  }
  try {
...
async function withBrowserSession({ platform, account, profile, cdp, useStealth = true }, callback) {
  const config = BROWSER_AUTH[platform];
  if (!config) {
    throw new Error(`Browser auth is not configured for ${platform}.`);
  }

  const { chromium } = await loadPlaywright({ useStealth });
  ...
  if (cdp) {
    const endpoint = resolveCdpEndpoint(cdp);
    console.log(`Connecting to existing Chrome over CDP at ${endpoint}. Reusing your logged-in session.`);
    browser = await chromium.connectOverCDP(endpoint);
    const contexts = browser.contexts();
    context = contexts[0] || await browser.newContext();
  } else {
    userDataDir = resolveBrowserProfile(profile, platform);
    await mkdir(userDataDir, { recursive: true });
    context = await chromium.launchPersistentContext(userDataDir, buildBrowserLaunchOptions(platform, { withWebSecurity: true }));
  }
  ...
  try {
    ...
    console.log(`Browser auth opened ${platform}. Please log in manually if needed. Profile: ${userDataDir || "(CDP)"}`);

    for (let attempt = 0; attempt < 2; attempt += 1) {
      const platformCookies = await waitForBrowserLogin({ context, page, config, platform, userDataDir: userDataDir || "(CDP)" });
      try {
    ...
    if (browser) {
      // CDP mode: keep the user's Chrome open; do not call browser.close().
    } else {
      await context.close();
    }
  }
...
async function main() {
  ...
  const cdp = resolveCdpEndpoint(args.cdp);
  const useStealth = !args["no-stealth"];
  const dataset = await withBrowserSession({ platform, account: args.account, profile: args.profile, cdp, useStealth }, async ({ context, page, cookieHeader }) => {
